...
export const searchConfigs = async (gitDir = process.cwd(), logger) => {
  /**
   * Always search for possible config from the root git directory,
   * because might be outside it (for example, when inside a submodule or worktree)
   */
  const rootConfigPath = await loadConfig({ cwd: gitDir }).then(({ filepath }) => filepath || null)
  ...
  const absoluteFiles = [...cachedFiles, ...otherFiles].map((file) => join(gitDir, file))

  /** Sort possible config files so that deepest is first */
  const possibleConfigFiles = [rootConfigPath, ...absoluteFiles]
    .filter(Boolean)
